(function() {
  ...
  jasmineNode.TerminalReporter.prototype = {
    ...
    reportFailures_: function() {
      ...
      var groupedSpecs = _.groupBy(this.failures_, 'spec')
      var failureCount = 1;
      for (spec in groupedSpecs) {
        var indentLevel = 1;
        var failures = groupedSpecs[spec];
        var descriptionSegments = failures[0].descriptionSegments;
        for (var i = 0; i < descriptionSegments.length; i++) {
          this.printLine_(new Array(indentLevel).join(' ') + descriptionSegments[i]);
          indentLevel += 2;
        }
        var failureIndentation = new Array(indentLevel).join(' ');
        var stackIndentation = new Array(indentLevel + 2).join(' ');
        for (var i = 0; i < failures.length; i++) {
          ...
          if (this.includeStackTrace_) {
              var stackTrace = this.stackFilter_(failure.stackTrace);
              if (stackTrace) {
                // Remove first line if it matches the message
                var stackTraceLines = stackTrace.split('\n');
                if (stackTraceLines[0] == failure.message
                    || stackTraceLines[0] == ('Error: ' + failure.message))
    ...
    addFailureToFailures_: function(spec) {
      ...
      for (var i = 0; i < items_length; i++) {
        if (result.items_[i].passed_ === false) {
          ...
          descriptionSegments.push('it ' + spec.description);
          var parent = spec.suite;
          while (parent) {
            descriptionSegments.unshift(parent.description);
