var fs = require('fs');
var coffeestack = require('coffeestack');
...
function removeJasmineFrames(text) {
  ...
  if (/^(\s*at .* )\(timers\.js:\d+:\d+\)/.test(lines[lines.length - 1])) {
    lines.pop()
  }

  // Ignore all lines starting at the first call to
  // Object.jasmine.executeSpecsInFolder()
  var keepLine = true;
  lines = lines.filter(function(line) {
    if (keepLine && /^\s*at Object\.jasmine\.executeSpecsInFolder/.test(line)) {
      keepLine = false;
    }
    return keepLine;
  });

  return coffeestack.convertStackTrace(lines.join('\n'), sourceMaps);
}
